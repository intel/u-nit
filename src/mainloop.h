...
struct mainloop_timeout *mainloop_add_timeout(uint32_t msec, enum timeout_result(*timeout_cb)(void));
void mainloop_remove_timeout(struct mainloop_timeout *mt);

struct mainloop_signal_handler *mainloop_add_signal_handler(sigset_t *mask, void (*signal_cb)(struct signalfd_siginfo *info));
