...
static void
add_entry_to_list(struct inittab_entry **list, struct inittab_entry *entry)
{
    ...
    if ((*list == NULL) || ((*list)->order > entry->order)) {
        ...
        *list = entry;
        ...
        current = *list;
        while ((current->next != NULL) && (current->next->order <= entry->order)) {
...
static void
debug_inittab_entry_list(struct inittab_entry *list)
{
    if (list == NULL) {
        log_message("\tNULL\n");
    } else {
...
static void
debug_inittab_entries(struct inittab *inittab_entries)
{
...
void
free_inittab_entry_list(struct inittab_entry *list)
{
...
static bool
place_entry(struct inittab_entry *entry, struct inittab *inittab_entries)
{
    bool result = true;
...
bool
read_inittab(const char *filename, struct inittab *inittab_entries)
{
    FILE *fp = NULL;
    ...
    while (true) {
        ...
        if (entry == NULL) {
            log_message("Could not allocate memory to parse inittab entry\n");
            error = true;
            break;
        }
        ...
        if (r == RESULT_OK) {
            ...
            error = true;
            free(entry);
            /* TODO currently, `inittab_parse_entry` itself prints error. Maybe it'd
